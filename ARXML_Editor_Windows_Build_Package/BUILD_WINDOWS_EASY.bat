@echo off
echo ========================================
echo ARXML Editor - Easy Windows Build
echo ========================================
echo.
echo This script uses pre-compiled packages to avoid
echo compilation issues on Windows.
echo.
pause
...
REM Install packages using Windows-specific requirements
echo Installing packages using Windows-specific requirements...
pip install -r requirements_windows.txt
if errorlevel 1 (
    echo ERROR: Failed to install packages
    echo Trying with pre-compiled wheels only...
    pip install --only-binary=all -r requirements_windows.txt
    if errorlevel 1 (
        echo ERROR: Failed to install packages even with pre-compiled wheels
        pause
        exit /b 1
    )
)
